		...
		try{
			...
			if(a.stackSize==0||(a.getItemDamage()==a.getMaxDamage()&&a.getItem().getItemStackLimit()==1)){
				...
				for(int i=0; i<stacks.length; i++){
					if((a.getItem().equals(stacks[i].getItem())&&a.getItemDamage()==stacks[i].getItemDamage())){
						a=stacks[i];
						stacks[i]=null;
	...
	private void writeObjectToNbt(ItemStack inside, ItemStack outside){
		try{
			...
			if(inside!=null){
				...
				NBTTagCompound combinedTag=new NBTTagCompound();
				combinedTag.setTag("spydata", futureTag);
